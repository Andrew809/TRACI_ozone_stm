...
# import pandas as pd
# used to be from gdal import osr; now, per https://pypi.org/project/GDAL/#:~:text=Usage-,Imports,-There%20are%20five
...
def get_geofile_id_col_fromfile(filenameID):
    ...
    # datarow = cfg.dict_excelTables[cfg.t_geo_files].loc[filenameID, :]
    # the_id = datarow[cfg.xlcols[cfg.t_geo_files][cfg.s_id]]

    the_id = cfg.df_files.loc[filenameID, cfg.tbl_files_col_headerID]
...
def read_shapefile_fromfile(geofilenameID, new_id_col='', rename_dict=None,
    ...
    if os.path.isfile(cfg.dir_input_pkls + os.sep + geofilenameID + '.pkl'):
        if cfg.bln_debug: print(f'\t\t\tReading <{geofilenameID}> from *pickle* in function <read_shapefile_fromfile>')
        shapefile = read_or_save_pickle('read', cfg.dir_input_pkls + os.sep + geofilenameID + '.pkl')
    else:
        if cfg.bln_debug: print(f'\t\t\tReading <{geofilenameID}> in function <read_shapefile_fromfile>')
        # we are reading from the geofile table
        file, path, ext = get_filepathext_from_files(filenameID=geofilenameID)
        shapefile = gpd.read_file(filename=path + file + ext)
        if cfg.bln_debug: print(f'\t\t\t\t... done reading.')

        if cfg.bln_save_geopickles:
            if cfg.bln_debug2: print(f'\t\t\tSaving the shapefile as a pickle...')
            read_or_save_pickle('save', cfg.dir_input_pkls + os.sep + geofilenameID + '.pkl',
                                list_save_vars=shapefile)
            if cfg.bln_debug2: print(f'\t\t\t\t...done saving.')
    ...
    # get current ID
    curID = cfg.df_files.loc[geofilenameID, cfg.tbl_files_col_headerID]
    ...
    if cfg.bln_debug: print(f'Checking <{geofilenameID}> for projection and area...>')
    shapefile = check_for_projection_and_area(shpfile_to_check=shapefile,
                                              new_area_col=new_area_col,
                                              proj_id=proj_id,
                                              geofilenameID=geofilenameID)
    if cfg.bln_debug: print(f'\t\t...Done.  Returning shapefile from calling function.')
    return shapefile
...
def get_projection(projnameID):
    ...
    datarow = cfg.dict_excelTables[cfg.t_proj].loc[projnameID, :]
...
def get_assoc_geofile_from_data(datanameID):
    ...
    if datanameID is None:
        ...
        return cfg.df_data.loc[datanameID, cfg.tbl_data_col_assocGeo]
        # # no check that datanameID is in index, but it is data validated in excel
        # datarow = cfg.dict_excelTables[cfg.t_data].loc[datanameID, :]
        #
        # # try the data column:
