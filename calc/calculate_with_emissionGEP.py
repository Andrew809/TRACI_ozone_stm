...
file_settings = r'acidification CF calc setup v00a.xlsx'
file_output = r'acidification CF output_v00a'  # will be written as excel file
...
for calc_idx, calc_row in cfg.df_calc.iterrows():
    ...
    if calc_row[cfg.tbl_calc_col_doCalc] != True:
        ...
        # totals
        # TODO: the 'combination' across the receiving regions should be averaged -- either implicit 1 weights or explicit weights
        CF_regional = np.average(FFxRFxEF, axis=1)
        CF_gep = np.average(FFxRFxEFxGEP, axis=1)

        # save the CFs into df
        df_CF = emission_Names
        ...
        for impactScale_idx, impactScale_row in cfg.df_impactScale.iterrows():
            ...
            if impactScale_row[cfg.tbl_impactScale_col_Include]==True: # thet we are recording this calc scale
                ...
                temp_multiindex = (calc_row[cfg.tbl_calc_col_emitResNew],  # aggregation Resolution
...
for emitResKeys in cfg.dict_output_by_emitRes.keys():
    ...
    if (cfg.str_sector_agric in list_sectors_used) and (cfg.str_sector_notAg in list_sectors_used):
        ...
        for aggRes in dfo.columns.unique(cfg.multiindex_col_emitResNew).to_list():
            ...
            for subst in dfo.columns.unique(cfg.multiindex_col_subst).to_list():
                ...
                for impScale in dfo.columns.unique(cfg.multiindex_col_impactScale).to_list():
                    ...
                    if (dfo.columns.isin([(aggRes, subst, impScale, cfg.str_sector_agric)]).any()) and (
                    dfo.columns.isin([(aggRes, subst, impScale, cfg.str_sector_notAg)]).any()):

                        if cfg.bln_debug:
                            print(f'\t\tCalculating general weighting; we can do so for {aggRes, subst, impScale}')

                        temp_agric = dfo.loc[:, (aggRes, subst, impScale, cfg.str_sector_agric, cfg.multiindex_col_dataItem_CF)]
                        temp_nonag = dfo.loc[:, (aggRes, subst, impScale, cfg.str_sector_notAg, cfg.multiindex_col_dataItem_CF)]

                        temp_genrl = (temp_agric + temp_nonag) / 2

                        dfo.loc[:,(aggRes, subst, impScale, cfg.str_sector_genrl, cfg.multiindex_col_dataItem_CF)] = temp_genrl
                        # also record the names and IDs.  Can take from either agric or nonagric.
                        dfo.loc[:, (aggRes, subst, impScale, cfg.str_sector_genrl, cfg.multiindex_col_dataItem_Name)] = \
                            dfo.loc[:, (aggRes, subst, impScale, cfg.str_sector_agric, cfg.multiindex_col_dataItem_Name)]
                        dfo.loc[:, (aggRes, subst, impScale, cfg.str_sector_genrl, cfg.multiindex_col_dataItem_Code)] = \
                            dfo.loc[:, (aggRes, subst, impScale, cfg.str_sector_agric, cfg.multiindex_col_dataItem_Code)]
                        if cfg.bln_debug2:
                            print(f'\t... new general: ')
                            print(dfo.columns)

        # so everything is grouped by substance, etc., sort the columns by the indices
        #dfo.columns = dfo.sort_index(axis=1, level=[0, 1, 2, 3, 4], ascending=[True, True, True, True, True]).columns
        dfo = dfo.sort_index(axis=1, level=[0, 1, 2, 3, 4], ascending=[True, True, True, True, True])
    ...
    # now DFO has 5-level multiindex columns. The last level, 'Data Item' has CFs, location names, and location IDs.
    ...
    df_stack = dfo.stack(level=[cfg.multiindex_col_emitResNew, cfg.multiindex_col_subst, cfg.multiindex_col_impactScale, cfg.multiindex_col_sectorWeight], future_stack=True)
...
write_df_to_excel(df=cfg.df_out_final, file = file_output + '_' + cfg.suffix_time, path = cfg.dir_output, ext='.xlsx', merge_cells=False)
